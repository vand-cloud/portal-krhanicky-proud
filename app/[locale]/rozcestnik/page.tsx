// ⚠️ DEPLOY DECISION PENDING ⚠️
//
// This route was the original homepage for /. Since {2026-05-05} the
// homepage is the search-driven Krhanický průvodce (see app/[locale]/
// page.tsx). The old curated 3-column layout (upcoming events, featured
// services, village news) lives here at /rozcestnik so we keep the
// pattern available, but it is NOT linked from the site navigation.
//
// Before /launch-web: decide one of three paths
//   a) DELETE this file -- pattern is unused, no value to keep dead code
//   b) PROMOTE -- if Simon decides the curated rozcestník is a useful
//      secondary surface, link it from the site nav and update copy
//   c) REPURPOSE -- merge selected sections into another page
//
// The reminder also lives in HANDOVER.md under the "Pre-launch decisions"
// section so /launch-web does not silently ship dead code to production.
...
export default async function RozcestnikPage({
  params,
}: {
