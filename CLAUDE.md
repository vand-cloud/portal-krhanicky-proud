...
Tohle je klientský web pro **Krhanický Proud** (krhanicky-proud.cz). Postavený byl na ANFILOV Studio template, ale **provozní infrastruktura žije plně pod klientem** (Ivan Dvořák, org `vand-cloud`): GitHub repo, Sanity projekt, Vercel Hobby projekt. Studio sem už zdroje neposílá, jen pracuje na kódu jako collaborator.
...
## Role a vlastnictví (model B — full handover, aktivní od května 2026)

| Role | Osoba | Co vlastní / dělá |
|---|---|---|
| **Realizátor** | **Simon Anfilov** (`Anfilov` na GitHubu, `simon@anfilov.cz`) | Staví strukturu, web, taxonomii, design, content. Píše kód, commituje a pushe. Účet collaborator na klientových zdrojích. |
| **Provozovatel + správce + majitel** | **Ivan Dvořák** (org `vand-cloud` na GitHubu) | Vlastní doménu, GitHub repo, Sanity projekt (4nb8kl4e), Vercel Hobby projekt, později Resend účet. Spravuje obsah v Sanity Studiu jako Administrator. |

Praktické důsledky pro práci v repu:

- **Lokální git config** zůstává `Simon Anfilov` / `simon@anfilov.cz`. Vercel **Hobby** plán nemá filtr na commit autora (to bylo Pro pravidlo), takže Simon může pushovat na klientovo repo bez restrikcí, dokud má rolu `Write` collaborator.
- **Vercel Hobby je non-commercial** — pro občanský rozcestník OK; pokud by se to časem komerčně škálovalo (placená inzerce, transakce), je nutné přejít na Pro nebo Enterprise.
- **Jediný `origin`** ukazuje na klientovo repo. Žádný studio mirror, žádné dvojí pushování.
- **Push checkpoint** je dál explicitní akt prezentace klientovi (viz Hard rules) — push triggeruje Vercel auto-deploy a Ivan dostane preview URL.


## Repo a git remoty

Jediný remote, klient ho vlastní:

| Remote | URL | Účel |
|---|---|---|
| `origin` | `https://github.com/vand-cloud/portal-krhanicky-proud.git` | **Klientovo repo, drží Ivan, propojené s jeho Vercel Hobby pro auto-deploy.** Simon je collaborator s Write rolí. |

Workflow při dokončené iteraci, kterou Simon explicitně schválil:

```bash
git push origin main   # → Ivanův GitHub → Ivanův Vercel auto-deploy → preview URL
```

Pro přístup do klientových zdrojů Simon používá:

| Zdroj | Přístupová úroveň | Jak |
|---|---|---|
| GitHub repo | Outside collaborator (Write) | osobní GitHub PAT z `gh auth status`, scope `repo` |
| Sanity projekt 4nb8kl4e | Administrator | login `simon@anfilov.cz` přes [sanity.io/manage](https://www.sanity.io/manage/project/4nb8kl4e) |
| Vercel Hobby projekt | Collaborator (až ho Ivan přidá) | login Vercel účtem `Anfilov`, viz IVAN_SETUP.md |

Detailní handover protokol: viz [HANDOVER.md](./HANDOVER.md). Setup checklist pro Ivana: viz [IVAN_SETUP.md](./IVAN_SETUP.md).
...
### Git user
Lokální `git config user.name` a `user.email` v této složce zůstávají `Simon Anfilov` a `simon@anfilov.cz`. Klient má **Vercel Hobby**, který nemá filtr na commit autora (na rozdíl od Pro), takže autorství commitů zůstává Simonovi po celou dobu implementace. Klient si ho přidal jako outside collaborator na svém repu, push funguje přes osobní GitHub PAT.
...
Globální git config zůstává nedotčený, nastavení je jen lokální (per-repo).
...
- **Sanity Manage** (klient, projekt `4nb8kl4e`): https://www.sanity.io/manage/project/4nb8kl4e
- **GitHub Repo** (klient, `origin`): https://github.com/vand-cloud/portal-krhanicky-proud
- **Vercel Project** (klient, Hobby): URL doplníme po Ivanově importu projektu
- **Resend** (zatím neaktivní, doplníme až formuláře): TBD — pravděpodobně Ivan Free účet
